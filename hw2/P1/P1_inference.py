...
class DDPM(nn.Module):
    ...
    def sample(self, n_sample, size, device, guide_w=0.0):
        ...
        for i in range(self.n_T, 0, -1):
            ...
            if i % 50 == 0 or i == self.n_T or i == 1:
                x_i_store.append(x_i.detach().cpu().numpy())
                save_image(x_i, "P1/output_inference/" + f"z{i}.png")

        x_i_store = np.array(x_i_store)
        subarray = x_i_store[:, 0, :, :, :]  # 取第0個子陣列，形狀為 (40, 3, 28, 28)
        subarrays = [
            torch.from_numpy(subarray[i, :, :, :])
            for i in range(9)
            if i not in (1, 3, 5)
        ]
        x_concat = torch.cat(subarrays, dim=2)  # 將行張量連接成最終的 x_concat 張量
        save_image(x_concat, "P1/output_inference/" + "evolution_img.png")
...
def output_img():
    ...
    save_dir = "P1/output_inference/"
    pt_dir = "P1/output/model_99.pth"
    ...
    with torch.no_grad():
        ...
        for i in range(n_sample):
            ...
            save_image(x_gen[i], save_dir + f"{i%10}_{int((i-i%10)/10)+1:03d}.png")

        x_concat = torch.cat(x_concat_list, dim=2)  # 將行張量連接成最終的 x_concat 張量
        save_image(x_concat, save_dir + "concat_img.png")

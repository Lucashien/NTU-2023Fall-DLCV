...
class QueryVideoDataset(Dataset):
    def __init__(self,
        ...
        self.clip_dir = args.clip_path
        self.video_dir = args.clip_path
        self.split = split
        #self.data_dir = '/home/remote/mplin/DLCV/VQLoC/new_data/{}_images'.format(self.split)
        #self.data_dir = './images/{}_images'.format(self.split)
        #self.val_data_dir = '/home/remote/mplin/DLCV/VQLoC/new_data/val_images'
        #self.test_data_dir = '/home/remote/mplin/DLCV/VQLoC/test_images/'

        self.clip_reader = video_reader_dict[clip_reader]
        self._load_metadata(args)
        if self.split != 'train':
            self.annotations = self.annotations[::eval_vis_freq]


    def _load_metadata(self, args):
        UFS3_aug = args.Use_UFS_3
        UFS5_aug = args.Use_UFS_5
        UFS_gt_10_aug = args.Use_UFS_gt_10
        UFS_gt_20_aug = args.Use_UFS_gt_20
        UFS_test_aug = args.Use_UFS_test

        self.annotations, n_samples, n_samples_valid = [], 0, 0
        
        if(UFS3_aug == 'True' and self.split == 'train'):
            ann_file = os.path.join('./UFS/', 'UFS3_aug.json')
            with open(ann_file) as f:
                anno_json = json.load(f)
            for clip_n, clip_ann in anno_json.items():
                    for clip_anno in clip_ann['annotations']:
                        for qset_id, qset in clip_anno['query_sets'].items():
                            total = len(qset["response_track"])
                            for i in range(1, 3):#qset["response_track"]:
                                if "visual_crop_{}".format(i) in qset.keys():
                                    ...
                                    curr_anno = {
                                        ...
                                        "visual_crop": qset["visual_crop_{}".format(i)],
                                    ...
                                    #print(query_path)
                                    if clip_n == '859ed253-d752-4f1b-adc3-c76599117d6e':
                                        print(query_path)
                                    if os.path.isfile(query_path):
                                        self.annotations.append(curr_anno)
                                        n_samples_valid += 1
                                    elif self.split == 'train' and clip_n == '859ed253-d752-4f1b-adc3-c76599117d6e':
                                        print(query_path, curr_anno['clip_uid'], curr_anno['visual_crop']['frame_number'], curr_anno['visual_crop'])
                                    n_samples += 1

        if(UFS5_aug == 'True' and self.split == 'train'):
            ann_file = os.path.join('./UFS5/', 'UFS5_aug.json')
            with open(ann_file) as f:
                anno_json = json.load(f)
            for clip_n, clip_ann in anno_json.items():
                    for clip_anno in clip_ann['annotations']:
                        for qset_id, qset in clip_anno['query_sets'].items():
                            ...
                            for i in range(3, 5):#qset["response_track"]:
                                if "visual_crop_{}".format(i) in qset.keys():
                                    if not qset['is_valid']:
                                        continue
                                    response_track_frame_ids = []
                                    ...
                                    #break
        if(UFS_gt_10_aug == 'True' and self.split == 'train'):
            ann_file = os.path.join('./UFS_gt/', 'UFS_gt_aug.json')
            with open(ann_file) as f:
                anno_json = json.load(f)
            #self.annotations, n_samples, n_samples_valid = [], 0, 0
            for clip_n, clip_ann in anno_json.items():
                        ...
                        for qset_id, qset in clip_anno['query_sets'].items():
                            if len(qset["response_track"]) != 0:
                                ...
                                if os.path.isfile(query_path):
                                    ...
                                    print(query_path, curr_anno['clip_uid'], curr_anno['visual_crop']['frame_number'], curr_anno['visual_crop'])
                                n_samples += 1
        if(UFS_gt_20_aug == 'True' and self.split == 'train'):
            ann_file = os.path.join('./UFS_gt_2/', 'UFS_gt_aug_2.json')
            with open(ann_file) as f:
                anno_json = json.load(f)
            #self.annotations, n_samples, n_samples_valid = [], 0, 0
            for clip_n, clip_ann in anno_json.items():
        ...
        if(UFS_test_aug == 'True' and self.split == 'train'):
            ann_file = os.path.join('./UFS_test/', 'UFS_test_aug.json')
            with open(ann_file) as f:
                anno_json = json.load(f)
            for clip_n, clip_ann in anno_json.items():
                    for clip_anno in clip_ann['annotations']:
        ...
        if(args.Use_orig_data == 'True' or self.split == 'val'):
            ann_file = os.path.join(args.anno_dir, 'vq_{}.json'.format(self.split))
            with open(ann_file) as f:
                anno_json = json.load(f)
            #self.annotations, n_samples, n_samples_valid = [], 0, 0
            for clip_n, clip_ann in anno_json.items():
    ...
    def _get_query_path(self, sample):
        ...
        image_dir = './images/{}_images'.format(data_source)
        #if data_source == "train":
        #    image_path = os.path.join(self.data_dir, "{}/frame_{:07d}.png".format(clip_name, image_name + 1))
        #elif data_source == "val":
        #    image_path = os.path.join(self.val_data_dir, "{}/frame_{:07d}.png".format(clip_name, image_name + 1))
        #else:
        #    image_path = os.path.join(self.test_data_dir, "{}/frame_{:07d}.png".format(clip_name, image_name + 1))
        #image_path = os.path.join(self.data_dir, 'images_hw', "{}/frame_{:07d}.jpg".format(clip_name, image_name + 1))
        image_path = os.path.join(image_dir, "{}/frame_{:07d}.png".format(clip_name, image_name + 1))
        return image_path
    ...
    def __getitem__(self, idx):
        ...
        results = {
            ...
            'response_track_len': sample["response_track_valid_range"][1] - sample["response_track_valid_range"][0] + 1
        }
        return results

...
def train_epoch(config, loader, model, optimizer, schedular, scaler, epoch, output_dir, device, rank, wandb_run=None, ddp=True):
    ...
    for batch_idx, sample in tqdm.tqdm(enumerate(loader), total = len(loader)):
        ...
        clips, queries = sample['clip'].to(device), sample['query'].to(device)
        #with autocast():
...
def validate(config, loader, model, epoch, output_dir, device, rank, wandb_run=None, ddp=True):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(loader):
            ...
            results, preds_top = val_performance(config, preds, sample, device)
            try:
...
def val_performance(config, preds, gts, device, prob_theta=0.5):
    ...
    pred_bbox = torch.gather(pred_bbox, dim=1, index=top_idx.unsqueeze(-1).unsqueeze(-1).repeat(1,1,4)).squeeze().to(device)       # [b*t,4]
    pred_hw = torch.gather(pred_hw, dim=1, index=top_idx.unsqueeze(-1).unsqueeze(-1).repeat(1,1,2)).squeeze().to(device)           # [b*t,2]
    pred_center = torch.gather(pred_center, dim=1, index=top_idx.unsqueeze(-1).unsqueeze(-1).repeat(1,1,2)).squeeze().to(device)   # [b*t,2]
    ...
    gt_center = rearrange(gts['center'], 'b t c -> (b t) c').to(device)
    gt_hw = rearrange(gts['hw'], 'b t c -> (b t) c').to(device)
    gt_bbox = rearrange(gts['clip_bbox'], 'b t c -> (b t) c').to(device)
    gt_prob = gts['clip_with_bbox'].reshape(-1).to(device).to(device)
    gt_before_query = gts['before_query'].reshape(-1).to(device)

    # bbox loss
    #loss_center = F.l1_loss(pred_center[gt_prob.bool()], gt_center[gt_prob.bool()])
    loss_center = F.l1_loss(pred_center[gt_prob.bool().to(device)], gt_center[gt_prob.bool().to(device)])
    loss_hw = F.l1_loss(pred_hw[gt_prob.bool().to(device)], gt_hw[gt_prob.bool().to(device)])

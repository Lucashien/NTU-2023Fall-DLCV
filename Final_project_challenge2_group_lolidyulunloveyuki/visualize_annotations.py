...
def draw_box_on_image(image, box, color=(255, 0, 0), thickness=2):
    x1, y1, x2, y2 = [int(b) for b in box]
    image = cv2.rectangle(image, (x1, y1), (x2, y2), color=color, thickness=thickness)
    return image


def draw_border(image, color=(255, 0, 0), thickness=2):
    x1, y1 = 0, 0
...
def visualize_query_set(video_reader, qset, save_height=360):
    ...
    for rf_fno, rf_box in zip(rt_fnos, rt_boxes):
        ...
        rf_im = draw_box_on_image(rf_frame, rf_box, color=(0, 255, 0), thickness=2)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--annot-path", type=str, default = './annotated_pred.json')
    parser.add_argument("--clips-root", type=str, required=True, default=sys.argv[1], help = "Path to the clip directory")
